...
program define knits2
	...
	* Then we can check if first line is code or markdown.
	...
	local justfinished = 1 // assume first line is md
	file read `fin' line
	while !r(eof) {
		...
		* Delimiter ends
		if substr("`line'", 1, 6) == "```" {
		    local justfinished = 1
			file read `fin' line // read next line
		}
		
		* No delimiter and subsequent lines, just finished
		* Could be in_code_block or not.
		* TODO: `i' gets erased  in the trial foor loop
		if substr("`line'", 1, 6) != "```{s}" & `justfinished' {
		    *file write `fout' `"`=ltrim("`line'")'"' _n
			local justfinished = 0
			file write `fout' "/*" _n
			file write `fout' "`line'" _n
			file read `fin' line // read next line
		}
		* No delimiter and subsequent lines, not just finished
		if substr("`line'", 1, 6) != "```{s}" & !`justfinished'{
		    file write `fout' "`line'" _n
			file read `fin' line // read next line
		}
