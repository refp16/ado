	...
	* Then we can check if first line is code or markdown.
	...
	while !r(eof) {
		...
		* Get initial characters
		local init = substr(`"`macval(line)'"', 1, 6)
		...
		* No delimiter and is not code
		if !inlist("`init'", "```{s}", "```") & !`iscode' {
		    *file write `fout' "* `line'" _n
			file write `fout' _asis `"* `macval(line)'"' _n //TODO: fix the space issue after the *. No space produces a better log file with more bibliography. Why?
	...
	* Flags for header and footer
	...
	while !r(eof) {
		...
		* Header and footer indicators
		local header = `line_no' <= 6
		local footer = ("`line'" == "end of do-file" & !`header') | `footer'
        
		if substr("`line'", 1, 4) == ". * " {
		    local nline = subinstr("`line'", ". * ", "", 1)
